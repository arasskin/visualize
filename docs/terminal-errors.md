...
Each JSON line includes a server timestamp and the reported browser time, error message and stack, phase, pane ID, label, terminal dimensions and retry attempt count when available. The browser reports screen initialization, screen application, rendering, resizing, supervisor errors, input failures, and failed resize delivery, plus uncaught exceptions and rejected promises. Terminal output and typed commands are not recorded. Session tokens in URLs are redacted before transmission.

A failed terminal resize retries after 300 ms, 600 ms and 1.2 seconds. After four unsuccessful attempts, automatic resizing stops for that size and the pane displays `resize failed after 4 attempts`; its tooltip contains the underlying error. The final failure is logged with the requested dimensions and attempt count. Normal output, successful input and repeated notifications of unchanged geometry do not erase the error or renew the retry budget. A new size, a restored connection, a replacement session or reopening the pane starts a new attempt sequence. A successful resize clears the error; a replacement session clears errors belonging to its predecessor. Collapsing or disposing a pane cancels scheduled retries, and replies from superseded requests cannot overwrite the current size or error state.
