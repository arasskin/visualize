...
export function sourceReader(panel, id, file) {
  ...
  let revision = null; let stopped = false; let timer; let pending;
  async function refresh() {
    if (stopped || !panel.root.isConnected) return;
    try {
      if (!panel.shut && !document.hidden) {
        pending = new AbortController();
        const response = await fetch('/document?k=' + encodeURIComponent(window.TOKEN), {method:'POST', body:JSON.stringify({id, revision}), signal:pending.signal});
        if (!response.ok) throw new Error('Unable to read document file');
        const result = await response.json();
        if (stopped) return;
        if (typeof result.text === 'string') { const top = viewport.scrollTop; code.replaceChildren(); render(code, result.text, result.language || languageFor(file)); viewport.scrollTop = top; revision = result.revision; }
        status.textContent = '';
      }
  ...
  refresh();
  return {focus: () => viewport.focus({preventScroll:true}), stop() { stopped=true; clearTimeout(timer); pending?.abort(); viewport.remove(); }};
}
