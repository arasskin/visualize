...
export function configSearch(host, canvas, navigation) {
  ...
  let completion = {items: []}, option = -1;
  function closeList() {
    list.hidden = true; option = -1; input.setAttribute('aria-expanded', 'false'); input.removeAttribute('aria-activedescendant');
  }
  function complete() {
    closeList();
    completion = configSearchCompletions(model, input.value, input.selectionStart ?? input.value.length);
    completion.items = completion.items.slice(0, 12);
  ...
  function take(index) {
    ...
    input.focus({preventScroll: true}); closeList(); search();
  }
  function clear() {
  ...
  input.addEventListener('keydown', event => {
    ...
    if (event.key === 'Escape') {
      if (!list.hidden) { event.preventDefault(); event.stopPropagation(); closeList(); return; }
      event.preventDefault(); event.stopPropagation(); input.value = ''; search(); return;
    }
    if (event.key === 'Tab' && !event.shiftKey && !list.hidden) {
      event.preventDefault(); event.stopPropagation(); take(Math.max(0, option)); return;
    }
    if ((next || previous) && !list.hidden) {
      event.preventDefault(); event.stopPropagation();
      option = option < 0 ? (next ? 0 : completion.items.length - 1)
        : (option + (next ? 1 : -1) + completion.items.length) % completion.items.length;
      [...list.children].forEach((item, index) => item.setAttribute('aria-selected', String(index === option)));
      input.setAttribute('aria-activedescendant', list.children[option].id);
      list.children[option].scrollIntoView({block: 'nearest'}); return;
    }
    if (event.key === 'Enter' && !event.shiftKey && option >= 0 && !list.hidden) {
      event.preventDefault(); event.stopPropagation(); take(option); return;
    }
