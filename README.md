...
src/supervisor.janet
                    the live session -- pty, pump thread, backlog -- in a
                    process of its own (`visualize --supervise`), so
                    restarting the server keeps the agent running
