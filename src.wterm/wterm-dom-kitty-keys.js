...
const KEYPAD_BEGIN = { code: 1, final: "E" };
...
function printableEntry(event) {
    const current = codePoint(event.key);
    if (current === null || current < 0x20 || current === 0x7f)
        return null;
    const lower = event.key.toLowerCase();
    const primary = codePoint(lower);
    return { code: primary ?? current, final: "u" };
}
function resolveKey(event, normalizeKeypad) {
...
function hasModifier(prefix, pressedModifiers) {
...
function modifierValue(event, action, pressedModifiers) {
    let value = 1;
...
function legacyModifiedText(event) {
    if (event.metaKey)
        return null;
...
function legacyFunctionalSequence(functionalKey, flags, action, modifierBits, cursorKeysApp) {
    ...
    if (cursorKeysApp && legacyMode && nonLockModifiers === 0) {
        ...
        if (appSequence)
            return { handled: true, sequence: appSequence };
    }
...
export function encodeKittyKey(event, flags, action, pressedModifiers, cursorKeysApp = false) {
    if (flags === 0)
        return null;
    if (action === "release" && !(flags & KITTY_REPORT_EVENTS))
        return null;
    ...
    const legacyText = !(flags & KITTY_REPORT_ALL) &&
        textEntry !== null &&
        !event.altKey &&
        !event.ctrlKey &&
        !event.metaKey;
    const legacyModified = !(flags & 1) &&
    ...
    if (legacyModified !== null)
        return legacyModified;
    if (!entry)
        return null;
    if (entry.modifier && !(flags & KITTY_REPORT_ALL))
        return null;
    const modifiers = modifierValue(event, action, pressedModifiers);
    if (resolved?.functionalKey) {
        const legacy = legacyFunctionalSequence(resolved.functionalKey, flags, action, modifiers - 1, cursorKeysApp);
        if (legacy.handled)
            return legacy.sequence;
    }
    const eventType = flags & KITTY_REPORT_EVENTS
        ? action === "repeat"
            ? 2
            : action === "release"
                ? 3
    ...
    if (entry.final !== "u" && entry.final !== "~") {
        ...
        if (modifiers > 1)
            return `\x1b[1;${modifiers}${entry.final}`;
    ...
    if (textEntry && flags & KITTY_REPORT_ALTERNATES && event.shiftKey) {
        ...
        if (shifted !== null && shifted !== entry.code)
            sequence += `:${shifted}`;
    }
