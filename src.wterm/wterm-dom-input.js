...
export class InputHandler {
    constructor(element, onData, getBridge, getCellSize = () => null) {
        ...
        this._onSelectionUp = (event) => {
            const press = this.pendingMousePress;
            this.pendingMousePress = null;
            this.element.ownerDocument.defaultView?.removeEventListener("mouseup", this._onSelectionUp);
            if (!press || !this.element.ownerDocument.getSelection()?.isCollapsed ||
                Math.hypot(event.clientX - press.clientX, event.clientY - press.clientY) > 4)
                return;
            this.handleMouse(press, "press");
            this.handleMouse(event, "release");
            this.stopMouseCapture();
        };
        this._onMouseDown = (event) => {
            if (event.button === 0 && event.target instanceof Element && event.target.closest(".term-link")) return;
            if (event.button === 0 && !event.altKey && !event.shiftKey && this.getBridge()?.mouseTracking?.()) {
                this.pendingMousePress = event;
                this.element.ownerDocument.defaultView?.addEventListener("mouseup", this._onSelectionUp);
                return;
            }
    ...
    handleMouse(event, kind) {
        ...
        if (kind === "wheel") {
            const wheel = event;
            const mode = wheel.deltaMode;
            const horizontal = Math.abs(wheel.deltaX) > Math.abs(wheel.deltaY);
            const delta = horizontal ? wheel.deltaX : wheel.deltaY;
            if (!Number.isFinite(delta) || delta === 0)
                return;
            event.preventDefault();
            const direction = Math.sign(delta);
            const now = wheel.timeStamp;
            let remainder = this.wheelRemainder;
            if (!remainder || remainder.horizontal !== horizontal || remainder.direction !== direction ||
                remainder.modifiers !== modifiers || now - remainder.time > 200) {
                remainder = { horizontal, direction, modifiers, amount: 0 };
                this.wheelRemainder = remainder;
            }
            remainder.time = now;
            remainder.amount += Math.abs(delta) * (mode === 1 ? 1 : mode === 2
                ? (horizontal ? bridge.getCols() : bridge.getRows())
                : 1 / (horizontal ? charWidth : rowHeight));
            repeats = Math.floor(remainder.amount + 1e-9);
            remainder.amount = Math.max(0, remainder.amount - repeats);
            if (!repeats)
                return;
            code = (horizontal ? direction < 0 ? 66 : 67 : direction < 0 ? 64 : 65) | modifiers;
        }
        else {
        ...
        event.preventDefault();
        this.onData(`\x1b[<${code};${col};${row}${final}`.repeat(repeats));
    }
    stopMouseCapture() {
        this.mouseButtons = 0;
        const view = this.element.ownerDocument.defaultView;
