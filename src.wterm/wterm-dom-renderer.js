...
import { textLinks } from "./wterm-dom-hyperlink.js";
...
export class Renderer {
    ...
    _buildRowContent(rowEl, getCell, lineLen, cursorCol, rowIndex) {
        let text = "";
        const offsets = [];
        for (let col = 0; col < Math.min(lineLen, this.cols); col++) {
            const cell = getCell(col);
            offsets.push(text.length);
            if (cell.width !== 0) text += cell.chars ?? (cell.char >= 32 ? String.fromCodePoint(cell.char) : " ");
        }
        const urls = [];
        for (const { index, uri } of textLinks(text)) {
            if (!safeLinkHref(uri)) continue;
            for (let col = 0; col < offsets.length; col++) {
                if (offsets[col] >= index && offsets[col] < index + uri.length) urls[col] = uri;
            }
        }
        let html = "";
        let runStyle = "";
        ...
        for (let col = 0; col < this.cols; col++) {
            ...
            const cellLinkKey = inBounds ? linkIdentity(cell) || (urls[col] ? `url\0${urls[col]}` : "") : "";
            const cellLinkUri = inBounds ? cell.linkUri || urls[col] : undefined;
            if (inBounds && width === 0) {
                flushRun(col);
    ...
    syncScrollback(core, viewport) {
        ...
        if (selectionInContainer && this._scrollbackRowEls.length > 0) {
            ...
            end = Math.max(end, Math.min(scrollbackCount, this._scrollbackStartKey -
                discardedCount +
                this._scrollbackRowEls.length));
        }
        const startKey = discardedCount + start;
        if (hasDiscardedCount &&
