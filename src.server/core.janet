...
(defn main [& args]
  ...
  (defn config-edit [sent]
    ...
    (when (and (truthy? (get sent "draw")) (index-of action ["run" "regenerate"]))
      (def [drawn-lines drawn-problems ok svg generation] (:call project-graph :draw (= action "regenerate")))
      (put response "lines" drawn-lines)
      (put response "problems" drawn-problems)
