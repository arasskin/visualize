...
(defn to-dot
  ...
  (defn node-line [node indent]
    (def name (node :name))
    (def label (string/replace-all "_" " " (node :label)))
    (def ink (node :ink))
    ...
    (array/push out
                (string indent "\"" (quoted name) "\""
                        (attrs
                          [["label" (if-let [markup (label-markup label)]
                                       (raw markup)
                                       (quoted label))]

                           ["width" (string/format "%.3f" (label-width label))]
                           ["height" (string/format "%.3f" (label-height label))]
                           ["color" ink]
                           ["fontcolor" ink]

                           ;(if (node :file)
                              [["URL" (quoted (string "visualize-file:" (node :file)))]]
                              [])
  ...
  (defn emit-box [key depth]
    ...
    (array/push out (string pad "  label=\"" (quoted (string/replace-all "_" " " key)) "\"; style=dashed;"
                            " color=\"" hue "\";"
                            " fontcolor=\"" hue "\";"
                            " fontsize=11;"))
...
(defn draw
  ...
  (try
    [true (trimmed-svg (graphviz/render dot) (folded-inks graph))]
    ([err]
      [false (string "Graphviz could not draw this graph: " err)]))))
