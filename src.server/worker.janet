  ...
  (defn handle [op sent]
    (case op
      ...
      (let [lines (config/read-config path)]
        (config/write-config path (config/remember-placements lines sent))
        true)
      :markdown
      (let [lines (config/read-config path)]
        (config/write-config path (config/remember-markdown lines sent))
        true)
      :label
      (do
        (def lines (config/read-config path))
      ...
      (do
        ...
        (def target (or (get sent "file") path))
