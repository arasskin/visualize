...
(defn- run [[root requests replies]]
  ...
  (defn refresh []
    ...
    (def next (scan/fingerprint root false (hidden lines) path))
    (def changed (not= next fingerprint))
    (when changed
      (set fingerprint next)
      (set tree nil))
    (when (or changed (not= commands (tuple ;lines)))
      (when (not= commands (tuple ;lines)) (set tree nil))
  ...
  (defn handle [op sent]
    (case op
      ...
      (do
        (refresh)
        (unless tree (set tree (scan/scan root nil (hidden commands))))
      ...
      (do
        ...
        (unless (and drawing (deep= key drawing-key))
          (unless tree (set tree (scan/scan root nil (hidden lines))))
