  ...
  (each line (string/split "\n" text)
    (cond
      ...
      (when-let [hit (peg/match dependency line)
        ...
        (when current
          (def to (note label))
          (array/push dependencies [current to label])
          (unless (= to current) (array/push edges [current to]))))

      (when-let [hit (peg/match heading line)
                 label (first hit)]
        (set current (note label))
        (array/push headings current))))

  {:nodes nodes :edges edges :headings headings :dependencies dependencies :extension (names/extension (or path ""))})
